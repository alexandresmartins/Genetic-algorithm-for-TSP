...
total = "list"
total = 0
#Esta repetição irá executar o algoritmo 50 vezes
for (i in 1:50){
  cat("\nEssa é a execução ", i," para 50 gerações.")
  #Limpa o Environment do RStudio
  ...
  numeroGeracoes = 49 #Número de gerações
  ...
  total = c(total, (ag@melhorSolucao@notaAvaliacao))
}

#Plota um gráfico com a melhor solução de cada execução
plot(
  x = 1:51,
  y = total,
  type = "l",
  main = "Gráfico do valor de fitness para 50 execuções",
  col = "red",
  ylab = "Valor do fitness",
  xlab = "Número de execuções do algorito genético"
)